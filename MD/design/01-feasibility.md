...
- LLVMSharp 套件存在且運作於 .NET
- ⚠️ **要驗證**：LLVMSharp 在 LLVM 16+ 之後維護節奏不穩，有時與最新 LLVM 版本脫節
- **行動**：Phase 0 第一週就要做 spike test，固定一個明確的 LLVM 版本搭配（建議 LLVM 17 或 18）
...
- 上次主要更新時間需確認
- 若維護中斷，需要直接 P/Invoke libLLVM 或改用 ClangSharp
- Phase 0 必須驗證可用性
...
1. **第 1 階段**（~3 月）：JSON → LLVM IR CLI 工具獨立可用
2. **第 2 階段**（~6 月）：以此為基礎接 .NET 直譯器跑通 ARM 測試 ROM（不上 JIT）
