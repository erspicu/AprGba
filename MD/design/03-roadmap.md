...
## Phase 5：Memory Bus 收尾 — 給 PPU 用（1–2 週）
...
## Phase 8：最小 PPU + headless 截圖（1–2 週）— hand-written，不 JSON 化
...
| **M4** | Phase 5 完成（GBA memory map 收尾 + 商業 ROM 跑得起來） | — | ⏳ 下一步 |
| **M5** | Phase 8 完成（**最小 PPU + headless 截圖**，jsmolka 視覺驗證） | — | — |
| **M6 (可選)** | Phase 7 完成（block-JIT，效能優化） | — | — |

---

## 下一步：Phase 5 — GBA Memory Bus 收尾

Phase 4.5 已完工（2026-05-02）：兩顆完全不同的 CPU（ARM7TDMI 32-bit
+ LR35902 8-bit）都跑通 framework 的 spec→IR→JIT 主路。「換 CPU = 換
JSON」這個論點驗證完成。

下一步是 **Phase 5 — GBA Memory Bus 收尾（最小版）**，為 Phase 8 (PPU)
準備底層。GBA memory bus 已能載 jsmolka ROM；剩下 PPU 相關 IO 暫存器、
VBlank 中斷、DMA channel、HLE 最小 SWI set。**不**做 save file、商業
ROM 相容性、Timer 0-3 完整版。

之後路線：
- **Phase 8 (PPU + headless 截圖)** — hand-written `GbaPpu`，最小
  Mode 0/3 + sprite。CLI 學 apr-gb 設計：`apr-gba --rom=X --cycles=N
  --screenshot=Y.png`。**不做 GUI、不做 60fps loop**，純 headless 截圖
  跟 mGBA 對拍
- **Phase 7 (block-JIT) 變成可選優化** — 不影響 test ROM 截圖目標。
  想做的話晚一點再做（值得做的時機見 Phase 7 章節）

完成 Phase 5 + 8 後，GBA 端的 MVP 範圍（test ROM 視覺驗證）就收完。
繼續做下去屬於 stretch goals（商業遊戲相容、實時 GUI、APU 等）。
