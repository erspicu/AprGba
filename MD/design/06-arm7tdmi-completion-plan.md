...
>
> **完工數據**：
> - **159 個 xUnit 測試全綠**（含 CoverageTests vocabulary 與 mnemonic
>   baseline 強制檢查）
> - **44 個 ARM mnemonic + ~30 個 Thumb mnemonic** 全部能編出 LLVM IR
> - 所有 spec 中的 micro-op 都有 emitter；沒有 dead emitter
> - LLVM module 通過 `Verify`，0 diagnostics
> - Phase 2.6 通用化 refactor（R1-R5）內嵌完成於 Phase 2.5 期間

下面文件保留為**歷史 plan doc**，供 Phase 4.5 GB 移植時參考結構。
...
| 順位 | 編碼空間 | 主要指令 | 狀態 |
|---|---|---|---|
| 1  | Branch and Exchange      | BX                                              | ✅ |
| 2  | Multiply                 | MUL, MLA                                        | ✅ |
| 3  | Multiply Long            | UMULL, UMLAL, SMULL, SMLAL                      | ✅ |
| 4  | Single Data Swap         | SWP, SWPB                                       | ✅ |
| 5  | Halfword/Signed DT       | LDRH, STRH, LDRSB, LDRSH                        | ✅ |
| 6  | PSR Transfer             | MRS, MSR (imm/reg)                              | ✅ |
| 7  | Data Processing          | 16 ops × 3 編碼變體（Imm、RegImmShift、RegRegShift） | ✅ |
| 8  | Single Data Transfer     | LDR, STR (含 byte/word/imm/reg/pre/post/wb)     | ✅ |
| 9  | Undefined                | (永遠觸發 UND 例外的 reserved 編碼)             | ✅ |
| 10 | Block Data Transfer      | LDM, STM (×4 modes ×S-bit ×writeback)           | ✅ |
| 11 | Branch / Branch with Link| B, BL                                            | ✅ |
| 12 | Coprocessor (3 種)        | CDP, MRC/MCR, LDC/STC（stub → UND）              | ✅ |
...
**Thumb 模式（Thumb-1 16-bit）— 19 種官方格式：全部 ✅**
...
| F19 | Long Branch with Link (兩半 BL) |
