...
| 29.3c | FLDZ + FSTP m32fp + FLD m32fp + FXCH ST(i) | Phase 29.3 data-movement extension: stack push (FLDZ D9 EE), memory store via f64→f32 conversion (FSTP m32fp D9 /3 mem), memory load via f32→f64 conversion (FLD m32fp D9 /0 mem), register swap (FXCH ST(i) D9 C8+i). Will add `29.3-fpu-roundtrip.com` test ROM: FNINIT → FLDZ → FSTP m32fp [scratch] → HLT; expected scratch = 0x00000000. |
