...
| **29.10 + 29.3c/d gap-fill** | m80fp + FST m32 + FLD ST(i) — ✅ **DONE 2026-05-16** | Per use request "希望將 29.3c/29.3d/29.6/29.10 規格功能實作補上": filled the remaining gaps from the data-movement track. (1) **D9 /2 mem = FST m32fp** (no-pop store; same path as FSTP m32 but skip Pop) via new `X86FpuHelpers.StoreMemF32` helper (DRY'd against FSTP m32). (2) **D9 C0-C7 = FLD ST(i)** (register-form push of logical ST(i)) via range check + LoadLogicalSt(rm) + Push. (3) **DB /5 = FLD m80fp** and **DB /7 = FSTP m80fp** (Phase 29.10) via new `LoadMemF80AsF64` / `StoreMemF80` helpers — bit-manipulation in LLVM IR converting 80-bit format (15-bit exp, 63-bit mantissa, explicit integer bit, bias 16383) to/from f64 (11-bit exp, 52-bit mantissa, implicit integer bit, bias 1023). Special cases zero (exp=0 → produce 0.0 / 0x0000 exp) and Inf/NaN (exp=0x7FFF → f64 exp=0x7FF) handled without branches via LLVM `select`. Phase 29.6 (constants) confirmed already covered by 29.3d (7 FLD1/FLDL2T/FLDL2E/FLDPI/FLDLG2/FLDLN2/FLDZ constants). Test ROM `29.10-fpu-fillgaps.com` runs 3 sub-tests (FST→FSTP π pair, FLD ST(i) of 3.5, FLD m80(e) → FSTP m32) — all 4 verification GPRs match expected (AX/BX=0x4049, CX=0x4060, DX=0x402D). FreeDOS HLE regression: 2838 INT calls. |
...
| 29.7 | FPU transcendentals (F2XM1/FYL2X/FPTAN/FPATAN + FDECSTP/FINCSTP) — ✅ **DONE 2026-05-16** | Per Gemini guidance, transcendentals go through C# `Math.*` externs (not LLVM intrinsics which are unreliable for f80 on non-x86 targets). 4 new extern names in `MemoryEmitters` (`fpu_tan` / `fpu_atan2` / `fpu_log2` / `fpu_exp2m1`) with `CallFpuUnary` / `CallFpuBinary` helpers; X86JsonCpu adds 4 `[UnmanagedCallersOnly]` shims routing to `Math.Tan` / `Math.Atan2` / `Math.Log2` / `Math.Pow(2,x)-1`. D9 dispatcher gets 6 new cases: F0 F2XM1, F1 FYL2X (ST(1) = ST(1) * log2(ST(0)) then pop), F2 FPTAN (tan(ST(0)) → ST(0), then push 1.0), F3 FPATAN (atan2(ST(1), ST(0)) → ST(1) then pop), F6 FDECSTP (TOP--), F7 FINCSTP (TOP++). New helper `X86FpuHelpers.AdjustTop(delta)` for FDECSTP/FINCSTP — bypasses Push/Pop semantics (no data/tag change, just TOP rotation). Test ROM `29.7-fpu-transcendental.com` runs all 4 transcendentals: F2XM1(0.5)=√2−1 → AX=0x3ED4 (high WORD of 0x3ED413CD), FYL2X(2.0, 8.0)=6.0 → BX=0x40C0, FPTAN(0) pushes 1.0 + tan(0)=0 → CX=0x3F80 DX=0x0000, FPATAN(1.0, 1.0)=π/4 → SI=0x3F49 (high WORD of 0x3F490FDB). All bit-exact f32 of glibc / .NET Math.* results. FreeDOS HLE regression: 2838 INT calls. |
