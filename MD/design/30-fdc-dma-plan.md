...
| 30.8 | MDA mode "some chars invisible" — confirmed via Gemini 20260516_220912.txt + pcxtbios.asm source as **a real pcxtbios bug** (`int_10_func_14` teletype implicit scroll uses BH=0 in text mode → new line attr=0 black-on-black). Same bug exists for CGA but cosmetically less visible. Mature emulators solve via SeaBIOS / authentic IBM BIOS / DOSBox-style HLE intercept of INT 10h. We've cloned SeaBIOS source under `ref/seabios/` as a future option but NOT switched (SeaBIOS expects modern PC peripherals we don't yet emulate). Renderer hack rejected as a fix because it would break DOS programs that intentionally use black-on-black (passwords, fade effects). **Status: documented known pcxtbios limitation**; `--video=cga` is the recommended visible mode. | 📋 known-issue, deferred |
