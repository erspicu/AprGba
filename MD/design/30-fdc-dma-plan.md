...
| **30.7a** | **GUI + real-BIOS keyboard end-to-end interactive A:\\>** — 16 sub-bug investigation (XLAT, HLT wake-on-IRQ, FDC motor stall hack, port 0x61 ack pulse, etc.) | ✅ `64cb41b` |
| **30.7a-followup** | 8250 UART + LPT printer status stubs (defensive) | ✅ `58e38a8` |
| **30.7b** | **Full GUI FreeDOS interactive — dir / ver visible end-to-end**: F12 framebuffer dump hotkey, KeyDown/KeyUp Shift modifier handling, attr=0 → 0x07 renderer workaround (later reverted in 30.7c) | ✅ `3d3c215` |
| **30.7c** | **`--video=mda\|cga` selector** + correct port 0x62 PPI mapping (per pcxtbios.asm source not IBM PC original) + reverted renderer attr=0 hack. CGA path 100% works (FreeDOS interactive A:\\> + dir + ver fully visible at 80x25). MDA path partial (palette correct, but pcxtbios POST chooses CGA over MDA because both VRAM probes pass) — `--video=cga` is the working recommendation. | ✅ `9dbb8fc` |
| **30.7d** | MDA-aware renderer — IBM 5151 green-phosphor palette + attribute byte pattern-match (invisible / underline / reverse / normal per real MDA hardware logic, not CGA 16-colour palette). MDA mode now renders monochrome correctly. Known limitation: chars on cells with attr=0 (uninitialised by POST CGA path) still render invisible — proper fix requires forcing pcxtbios to actually pick MDA mode 7. | ✅ (this commit) |
| 30.8 | MDA mode "some chars invisible" investigation. Initial diagnosis via Gemini was pcxtbios scroll bug — verified + partially fixed via Phase 30.10 below. Deeper root cause: FreeDOS CON driver does direct word-writes to MDA VRAM with attr=0 for some output paths (dir body specifically); not fixable from emulator side without HLE-intercepting word-writes to VRAM (out of scope). **`--video=cga` works fully**; MDA usable for typing but dir output partially invisible. | 📋 root cause identified, MDA-mode-specific limit |
| **30.10** | **HLE patches for pcxtbios INT 10h scroll bug** — (a) binary-patch BIOS `mov bh, ah` at F000:F6BE to `mov bh, 0x07` (line 4143 of pcxtbios.asm) with checksum filler compensation; (b) runtime intercept in `PcSystemRunner.EmulatorThreadProc` for any INT 10h AH=06 with BH=0 in text mode → force BH=0x07. Both target the "DOS reads bad attr from AH=08 → uses 0 for scroll" path. Effective for BIOS-internal scroll; doesn't help with FreeDOS direct VRAM writes (see 30.8). | ✅ this commit |
| **30.11** | **AutoTester — scripted GUI integration test framework**. CLI `--auto-test=<sequence>`. Polls framebuffer every 5s, matches patterns ("language", "[Y,N]", "A:\\>"), injects scancodes via `PcPortBus.InjectScancode`, dumps final screen + closes form. Built-in sequence `freedos-mda-dir` does full boot → language Enter → installer N+Enter → A:\\> dir+Enter → dump. Enables unattended bring-up testing without operator. `gui-test.bat realbios mda auto` to invoke. | ✅ this commit |
| **30.11b** | **FreeDOS source audit (zero direct VRAM writes) + permissive MdaDecodeAttr** (bg!=0→reverse, attr=0+printable→normal). Refutes Gemini's "FreeDOS direct VRAM word write" claim — MDA invisibility is BIOS scroll + installer CGA-only attrs. Cloned FreeDOS kernel + freecom to `ref/freedos/` as source-of-truth. Permissive renderer + 30.10 patch together make MDA `dir` fully visible. AutoTester gains PNG screenshot + per-row attr histogram. | ✅ `139e4a6` |
| **30.12** | **`--video-bios=PATH` option-ROM loader (smoke test)**. Loads `videorom.bin` (Tseng ET4000 32 KB VGA BIOS, V8.02X 1992) at `0xC0000`, validates `55 AA` + checksum. pcxtbios POST scans 0xC0000-0xFE000 for option ROMs (asm line 819-880) and FAR-CALLs offset 3 automatically. **Unexpected upshot**: the VBIOS init forces video mode → 3 (CGA 80x25 colour text at 0xB8000) instead of 7 (MDA), so all FreeDOS-installer 16-colour rendering "just works" via our existing CGA decoder. `caller=C000` shows 157 times in INT 10h trace — real Tseng init code runs. No VGA register / graphics-mode emulation needed for this milestone; smoke test is a hard pass with full colour FreeDOS installer + `dir` visible. | ✅ this commit |
| 30.9 | Speed up real-BIOS interactive (current ~500K inst/sec; `dir`/`ver` feel sluggish at idle due to FreeCom prompt `$P` redraw polling LBA 0 on every PIT tick). Real fix: block-JIT + FPU correctness. | ⏳ deferred |
| 30.10 | block-JIT + Phase 29 FPU correctness — i64 alloca slot for FPU_ST0 doesn't match f64 runtime layout; block-JIT produces black screen even with the slot type fix. Also unblocks Phase 28.8x (block-JIT INT trap loss). Biggest interactive-speed win available. | ⏳ deferred |
| 30.13 | (future) Phase 30.12 follow-up: emulate VGA register file (3C0-3CF, 3D4/3D5) + planar framebuffer (0xA0000) + mode 13h (320x200x256) + mode 12h (640x480x16) so VGA-aware DOS games can run. Out of scope until needed; current text-mode-3 path covers all FreeDOS needs. | ⏳ deferred |
| **30.14a** | **Port 0xE9 debug-out hook (Bochs/QEMU pattern)** — PcPortBus intercepts `OUT 0xE9, AL` → writes AL to `temp/port-e9.log` + mirrors to host stdout as `[E9] ...`. Smoke test: `test-roms/x86/src/30.14-port-e9-hello.asm` → `--test-rom=...` confirms hook fires. | ✅ this commit |
| **30.14b** | **`--floppy-b=PATH` second floppy mount** — plus equipment-word fix declaring 2 drives (PcPortBus `floppyCount` → SW2 nibble bits 2-3 → BDA[0x10] bits 6-7). FreeDOS now treats B: as a real drive instead of phantom-swap. | ✅ this commit |
| **30.14c** | **End-to-end NASM .COM via Port 0xE9 + B:** — `tools/make_fat12_floppy.py` pure-Python FAT12 1.44 MB builder (mtools unavailable on Windows; DiscUtils.Fat NuGet rejected as too heavy). NASM `HELLO.COM` on B: floppy. New AutoTester sequence `freedos-b-hello` auto-runs full boot → installer skip → B: → HELLO end-to-end. Surfaced bug: port 0x61 SW2 selector is bit 3 not bit 2 (pcxtbios `TURBO_ENABLED` writes 0xA5 = bit 2 sticky-high; bit 3 is the real PC/XT 8255 PIA select line). MainForm + AutoTester gained punctuation scancodes for `; : , . / \ [ ] ' ~ - = ` so shift+key works for typing `B:` etc. Workflow doc: [`MD/process/03-dos-test-injection-workflow.md`](../process/03-dos-test-injection-workflow.md). | ✅ this commit |
