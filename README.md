...
2. **Establish a perf baseline.** The legacy interpreter runs cpu_instrs at ~31 MIPS — faster than our current JIT. This is honest: **as of 2026-05-04, our LLVM block-JIT is still 13-32% slower than a well-tuned hand-coded interpreter**. We track this gap in `MD/performance/`.
...
Beyond "JSON in, working emulator out", these are the framework-level designs that took deliberate effort and are documented in `MD/design/`:
...
- **Three architectural patterns for timing-accurate block-JIT.** Predictive cycle downcounting (compute-once-deduct-as-you-go), MMIO catch-up callbacks (HW gets ticked at the moment it's observed), and sync exits (block ret-voids when HW state changes). Every timing problem is classified into one of these three. See `MD/design/15-timing-and-framework-design.md`.
...
1. **`MD/design/00-overview.md`** — what this project is at the highest level.
2. **`MD/design/02-architecture.md`** — how the pieces fit.
3. **`MD/design/12-gb-block-jit-roadmap.md`** — the active roadmap with every shipped commit + remaining items.
4. **`MD/design/15-timing-and-framework-design.md`** — the timing & framework-genericity synthesis. **Read this before touching any timing code.**
5. **`CLAUDE.md`** — project rules (commit QA workflow, scratch-file conventions, naming).
6. **`MD/process/01-commit-qa-workflow.md`** — what level of QA each kind of commit requires.
...
6. Document in `MD/design/` if you hit any framework-level surprises.
...
- **Timing model upgrades.** Per-cycle bus contention, deferred SMC invalidation, full pipeline modelling — each one fits one of the three architectural patterns in `MD/design/15`.
...
2. **建立 perf baseline。** Legacy interpreter 跑 cpu_instrs ~31 MIPS — 比我們現在的 JIT 快。誠實講：**截至 2026-05-04，我們的 LLVM block-JIT 仍比一個調過的手刻 interpreter 慢 13-32%。** 這個 gap 紀錄在 `MD/performance/`。
...
- **三個架構 pattern 處理 timing-accurate block-JIT。** Predictive cycle downcounting (先算總額邊跑邊扣)、MMIO catch-up callbacks (HW 在被觀測那刻才被 tick)、sync exits (HW state 改變時 block ret-void)。每個 timing 問題都歸到這三條軸的其中一條。詳見 `MD/design/15-timing-and-framework-design.md`。
...
1. **`MD/design/00-overview.md`** — 最高層次的「這個專案是什麼」。
2. **`MD/design/02-architecture.md`** — 各部分怎麼組合。
3. **`MD/design/12-gb-block-jit-roadmap.md`** — 目前 active roadmap、每個 ship 的 commit、剩下要做的。
4. **`MD/design/15-timing-and-framework-design.md`** — Timing 準確 + 框架通用化的 synthesis。**動任何 timing 相關 code 之前先讀這份。**
5. **`CLAUDE.md`** — 專案規則 (commit QA workflow / scratch-file 慣例 / 命名)。
6. **`MD/process/01-commit-qa-workflow.md`** — 哪種 commit 要過哪一級 QA。
...
6. 撞到任何框架級 surprise 的話寫進 `MD/design/`。
...
- **Timing 模型升級。** Per-cycle bus contention、deferred SMC invalidation、完整 pipeline 模擬 — 每個都套到 `MD/design/15` 三大 pattern 的其中一個。
