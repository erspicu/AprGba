// PNG encoder — copied from AprNes.Cli/Video/PngWriter.cs (same
// algorithm, namespace adjusted). Pure managed: DeflateStream +
// hand-rolled CRC32 + Adler32.
//
// We could lift this to AprCpu.Core to avoid copy-paste across the
// 4 CLI consumers (NES/GB/GBA/X86), but that's a small refactor for
// later — keep here for now to keep AprX86.Cli self-contained.
...
{
    /// <summary>Save packed RGB (3 bytes/pixel) as 8-bit truecolour PNG.</summary>
    public static void SavePng(byte[] rgb, int width, int height, string path)
    {
        if (rgb.Length != width * height * 3)
            throw new ArgumentException(
                $"framebuffer size mismatch: got {rgb.Length}, expected {width}×{height}×3 = {width * height * 3}",
                nameof(rgb));
        ...
        for (int y = 0; y < height; y++)
        {
            ...
            Buffer.BlockCopy(rgb, y * srcStride, raw, dst, srcStride);
            dst += srcStride;
        }
